...
def trainer(net, train_dataset, test_dataset, val_dataset, num_item, num_knapsack, max_epochs,
    ...
    net.eval()
    val_loss = 0
    for d_batch, w_batch, opt_sol, opt_value in loader_val:
        ...
        if model_type == "DYS":
            val_loss += criterion(w_batch, predicted[:,:-(num_knapsack + num_item)], opt_sol, opt_value, eval_mode=True).item()
        elif model_type == "Two-stage" or model_type == "SPO+" or model_type == "PertOpt-FY":
            val_loss += regret(w_batch, predicted, opt_sol, opt_value, eval_mode=True).item()
        elif model_type == "BBOpt":
            val_loss += criterion(w_batch, predicted, opt_sol, opt_value, eval_mode=True).item()
        elif model_type == "PertOpt":
            val_loss += criterion(w_batch, predicted, opt_sol, opt_value, eval_mode=True).item()
    ...
    while epoch <= max_epochs and train_time <= max_time:
        ...
        # Now compute regret on validation set
        net.eval()
        val_loss = 0
        for d_batch, w_batch, opt_sol, opt_value in loader_val:
            d_batch = d_batch.to(device)
            ...
            predicted = net(d_batch)
            if model_type == "DYS":
                val_loss += max(0., criterion(w_batch, predicted[:,:-(num_knapsack + num_item)], opt_sol, opt_value, eval_mode=True).item())
            elif model_type == "Two-stage" or model_type == "SPO+" or model_type == "PertOpt-FY":
                val_loss += regret(w_batch, predicted, opt_sol, opt_value, eval_mode=True).item()
            elif model_type == "BBOpt" or model_type == "PertOpt":
                val_loss += criterion(predicted, w_batch, opt_sol, opt_value, eval_mode=True).item()
        ...
        if val_loss < best_val_loss:
            ...
            for d_batch, w_batch, opt_sol, opt_value in loader_test:
                ...
                predicted = net(d_batch)
                if model_type == "DYS":
                    test_loss += max(0., criterion(w_batch, predicted[:,:-(num_knapsack + num_item)], opt_sol, opt_value, eval_mode=True).item())
                elif model_type == "Two-stage" or model_type == "SPO+" or model_type == "PertOpt-FY":
                    test_loss += regret(w_batch, predicted, opt_sol, opt_value, eval_mode=True).item()
                elif model_type == "BBOpt" or model_type == "PertOpt":
                    test_loss += criterion(predicted, w_batch, opt_sol, opt_value, eval_mode=True).item()
